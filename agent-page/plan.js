...
注意：**这一条只适用于 02 的核心目标**。03 的行动项不再重复写"为什么"，只讲具体做法（见"7. 严禁内容重复"）。
...
- **03 不重述理由**：03 只写具体做法，禁止再次解释机制或重复 02 的判断
...
全文正文字数控制在 **2400–3000 字**（不含标题与符号），各章节建议篇幅：
...
每个目标用列表呈现，控制在 4 行以内（**目标 + 为什么合在一起，不再分两个章节**）：
...
- **90天目标**：体重下降 3–5kg，建立每周 150 分钟活动量的习惯
...
- 不要为了量化而强行量化
- **此处只讲目标与理由，不写具体执行细节**（频率、数量、时间等具体做法全部属于 03）
...
- **全文控制在 2400–3000 字**（以"二、计划书结构"中的章节篇幅分配为准），避免大段长文
- 优先用短句（每句不超过 25 字）
