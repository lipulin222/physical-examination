...
· **加项价格（硬规则）**：加项价格**必须**取知识库第 7 章 7.3「项目单价表」的 T1 价，价格字段**只填数字**（不带 ¥ / 元 / 约，例如 750）。禁止因为"不确定"就写"另付"——**只有 7.3 中该项明确标为「—」「未标价」「免费」时，价格才填"另付"**，并在推荐理由里说明需门店报价。
· **加项名称对号入座**：加项卡的「名称」直接沿用 7.3 里的项目名，便于查价。常见对照：低剂量胸部CT = 胸部螺旋CT平扫（低剂量）¥750；乳腺钼靶 = DR双侧乳腺钼靶X光（女）¥600；HPV分型 = HPV-DNA分型检测（23种）¥570；宫颈细胞学/LCT = 液基超薄细胞学检测（LCT/Thin-PREP）¥420；阴道分泌物检查 = 阴道分泌物常规检查 ¥110；PSA = 前列腺特异抗原三项（男）¥360；心脏彩超 ¥800；颈动脉彩超 ¥500；骨密度 = DR骨密度测定（DEXA）¥600；人体成分分析 ¥250；甲状腺彩超 ¥400；乙肝五项定量 = 7.3 未标价（可写"另付"）。
...
例外：S5 初步推荐轮不要列选项——输出【套餐卡】后前端会自动展示固定引导文案与「继续提问」按钮；S7 最终推荐轮除外，须按 S7 规则列选项。`;
  ...
  // ② 小结提示词：生成《体检方案推荐小结》
  const SUMMARY_PROMPT = `你是卓正AI体检顾问。请根据对话记录，输出一份《体检方案推荐小结》，供用户预约购买前快速回顾。要求：
...
· 结尾固定附一句：本小结为健康管理建议，最终以医生面诊与检查安排为准`;
  ...
  // 金额解析容错：从 "¥750" / "750元" / "约 750" / "750" 中取出数字
  // 模型偶尔会带货币符号或单位，直接 parseFloat 会拿到 NaN 而退化显示成"另付"
